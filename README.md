...
### RU
```
*WebSocket* - это протокол компьютерных коммуникаций, обеспечивающий одновременный двусторонний канал связи через одно соединение с использованием протокола управления передачей данных (TCP). Иными словами - он позвяолет создавать интекративные соединения между клиентом и сервером для обмена сообщениями втреальном времении. 

HTTP и WebSocket ?
- HTTP - работает по схеме запрос - ответ, то есть клиент постоянно запрашивает какиую либо информацию, чтобы получить ответ. Сам же сервер не может оправить клиенут сообщение о какой то там готовности данных допустим, то есть, ему придется ждать когд аклиент сделать запрос чтобы то то ему отправить. 
- Socket - он работает немного иначе, тут получается между клиентом и сервером устанавливается постоянное соединение, но двунаправленное, то есть и клиент может серверу отправить сообщение и также сервер клиенту. Выходит так что клиент может постоянно слушать сервер и в какой то момент сервер отправлчет данных и клиент их сразу получает.

- Чаты
- Ленты соц сетей
- Многопользовательске сетевые игры
- Везде, где необходимо отклик в режиме реального времени
```
